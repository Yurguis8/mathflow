  ...
  const filteredTopics = useMemo(() => {
    return topics.filter((topic) => {
      ...
      const matchesDifficulty =
        ...
        normalizeText(topic.difficulty) === normalizeText(selectedDifficulty);
      ...
      <div className="flex flex-col md:flex-row gap-4 mb-8">
        ...
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 dark:text-white rounded-xl py-4 pl-12 pr-4 focus:outline-none focus:ring-2 focus:ring-brand-500 transition-all"
            value={search}
        ...
        <div className="relative w-full md:w-64 shrink-0">
          ...
          <select
            className="appearance-none w-full pl-12 pr-12 py-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl text-slate-600 dark:text-slate-300 font-medium focus:outline-none focus:ring-2 focus:ring-brand-500 cursor-pointer transition-colors"
      ...
      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {[1, 2, 3].map((i) => (
            <div
              key={i}
              className="bg-white dark:bg-slate-900 rounded-xl h-64 animate-pulse border border-slate-100 dark:border-slate-800"
            />
          ))}
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-xl p-8 sm:p-12 text-center border border-dashed border-slate-300 dark:border-slate-700">
